...
namespace Engine.Animation
{
    ...
    public class JointLimit
    {
        ...
        public static JointLimit FromDegrees(Vector3 minDeg, Vector3 maxDeg,
            EulerRotationOrder order = EulerRotationOrder.YXZ)
        {
            return new JointLimit
            {
                Min = new Vector3(
                    MathUtils.DegToRad(minDeg.X),
                    MathUtils.DegToRad(minDeg.Y),
                    MathUtils.DegToRad(minDeg.Z)),
                Max = new Vector3(
                    MathUtils.DegToRad(maxDeg.X),
                    MathUtils.DegToRad(maxDeg.Y),
                    MathUtils.DegToRad(maxDeg.Z)),
                RotationOrder = order
            };
        }
        ...
        /// 四元数转欧拉角
        /// </summary>
        private static Vector3 QuaternionToEulerAngles(Quaternion q, EulerRotationOrder order)
        {
        ...
        /// 欧拉角构建旋转矩阵
        /// </summary>
        private static Matrix ComposeFromEulerAngles(Vector3 angles, EulerRotationOrder order)
        {
            float yaw = angles.X;
            ...
            switch (order)
            {
                ...
                case EulerRotationOrder.XYZ:
                    return Matrix.CreateRotationX(pitch) * Matrix.CreateRotationY(yaw) * Matrix.CreateRotationZ(roll);

                case EulerRotationOrder.ZXY:
                    return Matrix.CreateRotationZ(roll) * Matrix.CreateRotationX(pitch) * Matrix.CreateRotationY(yaw);

                case EulerRotationOrder.ZYX:
                    return Matrix.CreateRotationZ(roll) * Matrix.CreateRotationY(yaw) * Matrix.CreateRotationX(pitch);
